...
export default function Discahrged(props) {
    ...
    useEffect(() => {
        ...
        Axios.get("/log-carga?origen=altas" + limite + offsetV)
            .then(response => {
    ...
    useEffect(() => {
        const limite = limit ? "&limit=" + limit : ""
        setOffset(0);
        Axios.get("/log-carga?origen=altas" + limite)
            .then(response => {
                setLogCarga(response.data)
            }).catch(e => {
                console.error(e)
    ...
    const handleFileChange = (dischargedFile) => {
        setErrors([])
        if (dischargedFile && dischargedFile.file && dischargedFile.file.data) {
            setFile(dischargedFile.file.data)
            setFileName(dischargedFile.name)
            return
        }
    ...
    const deleteUploadFile = () => {
        Axios.delete("/altas/" + fileUploadCode, {
            data: {
                fileUploadCode,
                userId: getUserId(),
            }
            ...
            console.error("Error al eliminar altas de carga: ", err)
            if (err.request.status === 403) {
                props.history.push('/login');
                return
            }
    ...
    return (
        <AdminLayout>
            ...
            >
                ...
                >
                    <h3 className={modalClasses.modalTitle}>Eliminar altas</h3>
                </DialogTitle>
                <DialogContent
                    id="modal-slide-description"
                    className={modalClasses.modalBody}
                >
                    <h4>Esta seguro que desea <b>eliminar</b> las altas relacionados a esta carga?</h4>
                </DialogContent>
                <DialogActions
