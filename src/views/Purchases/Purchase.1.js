    ...
    const validateFile = () => {
        ...
        for (let i = 1; i < purchases.length; i++) {
            ...
            if (!validateLength(serie, 19)) {
                errorsArray.push(`Error en la linea ${i}, el campo 'SERIE' no posee 19 caracteres.`)
            }

            if (!validateLength(dn, 9)) {
                errorsArray.push(`Error en la linea ${i}, el campo 'DN' no posee 9 caracteres.`)
            }

            if (!validateIntField(serie, 19)) {
                errorsArray.push(`Error en la linea ${i}, el campo 'SERIE' no es de tipo numerico.`)
            }

            if (!validateIntField(dn, 9)) {
                errorsArray.push(`Error en la linea ${i + 1}, el campo 'DN' no es de tipo numerico.`)
            }
            ...
            if (i === 0) {
                ...
                const serieRepited = validateRepited(processArray, "serie", serie)
                const dnRepited = validateRepited(processArray, "dn", dn)

                if (serieRepited) {
                    errorsArray.push(`Error en la linea ${i}, el campo 'SERIE' se encuentra repetido.`)

                }
                if (dnRepited) {
                    errorsArray.push(`Error en la linea ${i}, el campo 'DN' se encuentra repetido.`)
                }
                processArray.push(rowProcessed)
            }
        }
